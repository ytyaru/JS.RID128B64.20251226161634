...
describe("IdHead.decode 特殊なBits指定のテスト", () => {
    ...
    test("RIDのカスタムBits", () => {
        const head = IdHead.decode("RID256"); // 一件bits=256,radix=64に見えるが、bits=128,radix=256である
        expect(head.bits.random).toBe(128);
        expect(head.radix).toBe(256);
...
describe("IdHead クラスインスタンスとgetterのテスト", () => {
    test("プロパティアクセス", () => {
        ...
        expect(head.urlUnsafed).toBe(false);
        expect(head.visibled).toBe(false);
        expect(head.padded).toBe(false);
    });
});
