...
class IdHead {
    ...
    static encode(obj) {
        ...
        if (!bits || typeof bits.time !== 'number' || typeof bits.random !== 'number') throw new IdHeadEncordError(`bitsオブジェクトが不正です。bitsは'time'と'random'プロパティを持つオブジェクトであり、それぞれtypeofが'number'を返す値であるべきです。: has-time:${bits ? 'time' in bits : false}, has-random:${bits ? 'random' in bits : false}, is-number-time: ${bits ? typeof bits.time : 'undefined'}, is-number-random: ${bits ? typeof bits.random : 'undefined'}`);
