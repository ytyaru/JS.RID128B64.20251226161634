window.addEventListener('DOMContentLoaded', (event) => {
    ...
    const DSC = document.querySelector('#downloadSourceCode');
...
//        SRC.textContent = CODES[e.target.value];
    ...
    CSC.addEventListener('click', ()=>copyToClipboard(CODES[CTX.value]));
    DSC.addEventListener('click', ()=>downloadTextFile(CODES[CTX.value], `js/rid128b64.${CTX.value}`));
