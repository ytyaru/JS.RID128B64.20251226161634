...
class IdHead {
    static decode(headString) {
        ...
        if (type === 'T') {
            ...
            if (!Number.isInteger(bits.time) || !Number.isInteger(bits.random)) throw new IdHeadDecordError(`bitsは整数であるべきです。`);
            if (bits.time % 8 !== 0 || bits.random % 8 !== 0) throw new IdHeadDecordError(`bitsの合計は8の倍数であるべきです。`);
            if (bits.time < 48 || bits.random < 0) throw new IdHeadDecordError(`timeBitsは48以上、randomBitsは0以上必要です。`);
        } else {
            if (bitsPart && bitsPart.includes('-')) throw new IdHeadDecordError(`R/Iタイプではハイフン形式のbits指定はできません。`);
            bits.time = 0;
